...
import { SetMentorProfile, UpdateMentorProfileError } from "./MentorProfileActions";
...
export default function MentorProfile(props) {
  ...
  const {
    ...
    client_type_id,
    picture,
  } = useSelector((state) => state.mentorProfileReducer.data);

  const errors = useSelector(state => state.mentorProfileReducer.errors);
  ...
  const handleInputChange = (value, index, type) => {
    ...
    formInput.forEach((item) => {
      if(item.value || item.inputValue || item.selectValue) {
        ...
        dispatch(SetMentorProfile(formData));
      }
    });
    dispatch(UpdateMentorProfileError(updatedForm[index].name));
  }
